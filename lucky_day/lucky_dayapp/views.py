    ...
    def get_queryset(self):
        try:
            ...
            return self.model.objects.filter(~Q(
                offer_id__in=models.ScratchCard.objects.values('offer_id').filter(user_id=self.request.user))).order_by('-offer_id')[start:end]
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if offer.top_up_coin is not None:
                return Response({"status": 200, 'coin': offer.top_up_coin, "message": 'Scratch Card successfully.'},
                                status=status.HTTP_201_CREATED)

            if offer.cash is not None:
                return Response({"status": 200, 'cash': offer.cash, "message": 'Scratch Card successfully.'},
                                status=status.HTTP_201_CREATED)
        except Exception as e:
            print(e)
...
class GetLeaderboard(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        try:
            serializer = self.get_serializer(self.get_queryset(), many=True)
            try:
                user_rank = models.LeaderBoard.objects.values('rank_no', 'user_id__first_name', 'user_id__last_name',
                                                              'user_id__profile__coin',
                                                              'user_id__profile__profile_media').get(
